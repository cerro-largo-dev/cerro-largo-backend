...
import hashlib
from functools import wraps  # ← añadido
...
def hash_password(password):
    """Hash de la contraseña para mayor seguridad (no usado en este flujo simple)"""
...
def admin_login():
    ...
    try:
        ...
        if password == ADMIN_PASSWORD:
            # ⚑ Mantener el mismo flag histórico usado por tu cookie antigua
...
def check_auth():
    ...
    return jsonify({'success': True, 'authenticated': is_authenticated}), 200

def require_admin_auth(f):
    """Requerir autenticación de administrador"""
    @wraps(f)  # ← mantiene nombre/ayuda para Flask
...
def get_zone_states():
    """Obtener todos los estados de las zonas (público; si quieres, protégelo con @require_admin_auth)"""
    try:
